set term !! ;
create trigger TCRHMOD for CRCAR_HISTO
before update
position 0 as
/*
;/T/ Modification dans la table CRCAR_HISTO
;
...
*/
  if (    (   OLD.CRHTYPECR = "EN"
           or OLD.CRHTYPECR = "MC" )
      and OLD.CRHMNTCAR != NEW.CRHMNTCAR )
  then begin
    ...
    Update CRCLI_CIE Set CLCSLDACJ = CLCSLDACJ + OLD.CRHMNTCAR - NEW.CRHMNTCAR
                Where CLICIECLE = :t_refciecle
                  And CLICLE    = :t_refclenum
                  And CIECLE    = OLD.CIECLE;

    Update MCREF_SOLDE Set CLCSLDACJ = CLCSLDACJ + OLD.CRHMNTCAR - NEW.CRHMNTCAR
