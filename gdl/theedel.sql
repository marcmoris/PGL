set term !! ;
create trigger THEEDEL for MCHIS_ECR_ENG
Before delete
position 0 as

/* Destruction dans la relation MCHIS_ECR_ENG
;
...
*/
  if ( :t_coddc = "C" )
  then t_eng = (OLD.HEEMNTCRT - OLD.HEEMNTDBT);
  else t_eng = (OLD.HEEMNTDBT - OLD.HEEMNTCRT);
  ...
  then begin
    Update MCSAN_REST Set SRECUMENG1  = SRECUMENG1  - :t_eng01,
                ...
                Where SANANN    = :t_pecann
                  ...
                  And CPTCLE    = OLD.CPTCLE
                  And UNACLE    = OLD.UNACLE
                  And SRECIEINT = OLD.HEECIEINT
                  And SREUNAINT = OLD.HEEUNAINT;
